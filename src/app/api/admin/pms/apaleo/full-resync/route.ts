...
import { fullResyncApaleoConnection } from "@/lib/pms/apaleo-sync";
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid apaleo full-resync payload.", details: parsed.error.flatten() },
      { status: 400 }
    );
  }
  ...
  try {
    const result = await fullResyncApaleoConnection(connectionId, {
    ...
    const message = error instanceof Error ? error.message : "Unknown apaleo full resync error";
